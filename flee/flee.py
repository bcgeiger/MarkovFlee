...
class Person:
    __slots__ = ['location', 'home_location', 'timesteps_since_departure', 'places_travelled', 'recent_travel_distance',
                 'distance_moved_this_timestep', 'travelling', 'distance_travelled_on_link']
                ...
                if self.location.closed == True:
                    ...
                    if evolveMore == True:
                        ...
                        if SimulationSettings.AvoidShortStints:
                            if (self.recent_travel_distance + (
                                    self.distance_moved_this_timestep / SimulationSettings.MaxMoveSpeed)) / 2.0 < 0.5:  # Flee 2.0 Changeset 1, factor 2.
                                ForceTownMove = True
    ...
    Loops are avoided.
    """
        weight = float(self.getEndPointScore(link) / float(
            SimulationSettings.Softening + link.distance + prior_distance)) * link.endpoint.getCapMultiplier(
            link.numAgents)
        if debug:
            print("step {}, dest {}, dist {}, prior_dist {}, score {}, weight {}".format(step, link.endpoint.name,
        ...
        if SimulationSettings.AwarenessLevel > step:
            ...
            for k, e in enumerate(link.endpoint.links):
                if e.endpoint.name in origin_names:  # Link points back to an origin, so ignore.
                    ...
                    weight = max(weight, self.calculateLinkWeight(e, prior_distance + link.distance,
                                                                  origin_names + [link.endpoint.name], step + 1, debug))
...
class Location:
    def __init__(self, name, x=0.0, y=0.0, movechance=0.001, capacity=-1, pop=0, foreign=False, country="unknown"):
        ...
        self.linkprobs = []  # probability that a given link is chosen
        self.linkdistances = []  # distances of all connected links
        self.journey_endpoints = []  # possible journeys starting at this location
        self.journey_probs = []  # probabilities of respective journeys
        self.closed_links = []  # paths connecting to other towns that are closed.
        self.numAgents = 0  # refugee population
        self.numArrivingAgents = 0  # number of agents that will arrive in this time step
        self.numAgentsOnRank = 0  # refugee population on current rank (for pflee).
        self.capacity = capacity  # refugee capacity
        self.affected_locations = []  # if this is a camp, these are the locations from which journeys to/through it start
    ...
    def DecrementNumAgents(self, num=1):
        self.numAgents -= num

    def IncrementNumAgents(self, num=1):
        self.numAgents += num

    def IncrementArrivingAgents(self, num=1):
        self.numArrivingAgents += num
    ...
    def printNeighbors(self):
        for i in range(len(self.links)):
            print(self.links[i].endpoint.name)
    ...
    Loops are avoided.
    """
        weight = float(link.endpoint.scores[1] / float(
            SimulationSettings.Softening + link.distance + prior_distance)) * link.endpoint.getCapMultiplier(
            link.numAgents)
        ...
        if SimulationSettings.AwarenessLevel > step:
            ...
            for k, e in enumerate(link.endpoint.links):
                if e.endpoint.name in origin_names:  # Link points back to an origin, so ignore.
                    ...
                    weight = max(weight,
                                 self.calculateLinkWeight(e, prior_distance + link.distance,
                                                          origin_names + [link.endpoint.name],
                                                          step + 1, debug))

        return weight

    def calculateLinkProbs(self):
        '''
    This assumes that we leave the current location, i.e., all weights must actually be multiplied with
    movechance.
    '''
        ...
        weights = np.zeros(linknum)
        distances = np.zeros(linknum)
        for k, link in enumerate(self.links):
            weights[k] = self.calculateLinkWeight(link, 0.0, [self.name], 1, False)
            distances[k] = link.distance
        ...
        self.linkprobs = weights
        self.linkdistances = distances
        return
...
class Ecosystem:
    ...
    # @profile
    def evolveLocations(self,refresh_journey_probs=True):
        # update level 1, 2 and 3 location scores
        for l in self.locations:
            l.time = self.time
            l.updateLocationScore()
        ...
        if refresh_journey_probs:
            ...
            refresh_journey_probs = False

        # compute agent locations in next iteration
        for l in self.locations:
            dummy = np.random.multinomial(l.numAgents, l.journey_probs)
        ...
        self.time += 1
        return refresh_journey_probs

    def addLocation(self, name, x="0.0", y="0.0", movechance=SimulationSettings.DefaultMoveChance, capacity=-1, pop=0,
                    foreign=False, country="unknown"):
    ...
    """
        ...
        for i in range(0, len(self.agents)):
            if self.agents[i].location.name not in location_names:
                ...
                self.agents[
                    i].location.DecrementNumAgents()  # agent is removed from the ecosystem and number of agents drops by one.
    ...
    # @profile
    def computeRoutes(self, location, budget):
        '''
        ...
        if location.movechance > 0:
            for ind, link in enumerate(location.links):
                ...
                if link.distance < budget:
                    ...
                    affecting_camps=np.append(affecting_camps,next_camps)
                    if len(next_endpoints) > 0:
                        for ii in range(len(next_endpoints)):
                            probs = np.append(probs, location.linkprobs[ind] * location.movechance * next_probs[ii])
                            endpoints = np.append(endpoints, next_endpoints[ii])
                else:
                    endpoints = np.append(endpoints, link.endpoint)
                    probs = np.append(probs, location.linkprobs[
                        ind] * location.movechance)  # prob that you move there; you will stay with prob. 1.

        u_endpoints = []
        u_probs = []
        for ind in range(len(endpoints)):
            if endpoints[ind] in u_endpoints:
                u_probs[u_endpoints.index(endpoints[ind])] += probs[ind]
            else:
